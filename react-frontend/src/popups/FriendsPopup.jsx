...
function FriendsPopup({ toggleFriendsPopup }) {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        console.log(
          ...
          await getCurrUsername()
        );
        if (
          querySnapshot.docs[0].data().username === (await getCurrUsername())
        ) {
  ...
  async function removeFriend(email) {
    ...
    setFriends((prevFriends) =>
      prevFriends.filter((friend) => friend.email !== email)
    );
  }
      ...
      {selectedProfile && (
        <div className={styles.profileContainer}>
          <div className={styles.profileHeader}>
            ...
            />
            <h3 className={styles.profileUsername}>
